...
4. 自动更新时间在窗口里选星期和小时后点「保存时间」，再点「开启定时任务」。时间写在 TikView 表的「配置」工作表，默认每周一北京时间 10:00，默认关闭。电脑可以关着，由 GitHub 免费运行。GitHub 每小时看一次，只有开关开着、并且到了设定的北京时间，才会真正更新。第一次要用自动更新时，把项目推到 GitHub，并在仓库 Secrets 里配好上面那些密钥。
